...
\begin{itemize}
    ...
    \item File system (read \texttt{.aym}, write \texttt{.asm} and binaries).
\end{itemize}
...
Code generation & Validated AST & \texttt{.asm} \\
...
\begin{itemize}
    ...
    \item \textbf{FR-07} The system shall preserve an intermediate \texttt{.asm} artifact before linking.
    \item \textbf{FR-08} The system shall support imported modules from external files.
    \item \textbf{FR-09} The system shall include built-in functions (I/O, math, arrays, randomness).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{IR-02} Options: \texttt{--debug}, \texttt{--dump-ast}, \texttt{--seed}, \texttt{--windows}, \texttt{--linux}, \texttt{-o}.
    \item \textbf{IR-03} Output files: \texttt{.asm} (NASM) and native executable.
\end{itemize}
